...
class WebsocketGPSBroadcaster:
    ...
    async def broadcast_purchased_tickets_events(self, events: list[EventPaymentActivityCreate]):
        await self.ws_manager.broadcast(WSPaymentActivityEvents(type="purchased_tickets", items=events))
